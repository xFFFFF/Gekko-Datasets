...
while (@row = $sth->fetchrow_array()) {
  ...
  my $stmt = qq(SELECT high  FROM `main`.@row order by start ASC LIMIT 1;);
  my $sth = $dbh->prepare( $stmt );
  my $rv = $sth->execute();
  while (@row2 = $sth->fetchrow_array()) {
    print "lowest price: @row2\n";
  }
  my $stmt = qq(SELECT low  FROM `main`.@row order by start DESC LIMIT 1;);
  my $sth = $dbh->prepare( $stmt );
  my $rv = $sth->execute();
  ...
  my $stmt = qq(SELECT sum(trades)  FROM `main`.@row order by start DESC LIMIT 1;);
  my $sth = $dbh->prepare( $stmt );
  my $rv = $sth->execute();
  while (@row2 = $sth->fetchrow_array()) {
    print "trades: @row2\n";
  }
  my $stmt = qq(SELECT sum(volume)  FROM `main`.@row order by start DESC LIMIT 1;);
  my $sth = $dbh->prepare( $stmt );
  my $rv = $sth->execute();
  while (@row2 = $sth->fetchrow_array()) {
    print "volume: @row2\n\n";
